...
const UserList = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:8080/api/user/admin/users", {
          headers: {
        ...
        if (Array.isArray(result.data)) { // Change here
  ...
  useEffect(() => {
    if (filterRole === "") {
      ...
      const filtered = users.filter((user) => user.role && user.role.toLowerCase() === filterRole.toLowerCase());
  ...
  const handleSaveUser = async () => {
    try {
      ...
      const response = await axios.put(`http://localhost:8080/api/user/admin/edit-user/${editUser._id}`, editUser, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      if (response.data.success) {
        const updatedUsers = users.map((user) => user._id === editUser._id ? editUser : user);
  ...
  const handleDeleteUser = async (userId) => {
    try {
      ...
      const response = await axios.delete(`http://localhost:8080/api/user/admin/user/${userId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
        ...
        <table className="min-w-full bg-white border border-gray-200 shadow-md rounded-md">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Is Admin</th>
      ...
      {editUser && (
        ...
        >
          <div className="bg-white p-6 rounded-md shadow-lg w-full max-w-lg" >
            ...
            <div className="mb-4">
              <label htmlFor="isAdmin" className="block text-sm font-medium text-gray-700 mb-1">Is Admin</label>
              <select
                id="isAdmin"
